...
function repeatString(value, count) {
  if (count < 0) return '';
  if (value) return value.repeat(count);
  return value;
}
...
function removeFirstOccurrences(str, value) {
  ...
  if (index >= 0) return str.slice(0, index) + str.slice(index + value.length);
  return str;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < words.length; i += 1) {
    ...
    for (let j = 0; j < word.length; j += 1) {
      const ch = word[j];
      if (ch === ch.toUpperCase()) txt += ch.toLowerCase();
      else txt += ch.toUpperCase();
    }
    reverseStr.push(txt);
  }
  return reverseStr.join(' ');
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    const isAtoZ = charCode >= 65 && charCode <= 90;
    const isaTOz = charCode >= 97 && charCode <= 122;

    if (isAtoZ || isaTOz) {
      const find13 = s.toUpperCase() <= 'M' ? 13 : -13;
      const shift13 = charCode + find13;

      res += String.fromCharCode(shift13);
    } else res += s;
  }
  return res;
}
